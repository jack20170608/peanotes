 ...
 * {@link top.ilovemyhome.commons.common.codec.Hex Hex} and {@link Base64 Base64} codec classes to support the
...
public class SimpleByteSource implements ByteSource {
    ...
    public SimpleByteSource(File file) {
        if (file == null || !file.exists() || !file.isFile()) {
            throw new IllegalArgumentException("File argument cannot be null, must exist, and must be a file.");
        }
        this.bytes = ByteHelper.getBytes(file);
    }
    ...
    public SimpleByteSource(InputStream stream) {
        ...
        this.bytes = ByteHelper.getBytes(stream);
    }
