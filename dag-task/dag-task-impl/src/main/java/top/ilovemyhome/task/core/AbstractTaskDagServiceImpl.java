...
import top.ilovemyhome.peanotes.commons.jdbi.SearchCriteria;
import top.ilovemyhome.task.core.dag.DagHelper;
import top.ilovemyhome.task.core.dag.DagNode;
import top.ilovemyhome.task.si.*;
...
public abstract class AbstractTaskDagServiceImpl<I,O> implements TaskDagService<I,O> {
        ...
        this.loadingCache = Caffeine.newBuilder()
            ...
            .removalListener((key, value, cause) -> {
                if (cause == EXPIRED) {
                    ...
                    logger.info("Cache size limit reached for key: {}", key);
                } else {
                    logger.info("Cache removed for key: {} due to {}", key, cause);
                }
            }).build(key -> {
                logger.info("Loading cache for key: {}", key);
                List<Task<I,O>> tasks = loadByOrderKey(key);
                if (tasks.isEmpty()) {
                    throw new IllegalStateException("No tasks found for order key: " + key);
                }
                return tasks;
            });
        ...
        if (count > 0) {
            if (caseCade) {
                jdbi.useTransaction(h -> {
                    ...
                    //Delete in cache
                    loadingCache.invalidate(orderKey);
    ...
    public synchronized void start(String orderKey) {
        ...
        Optional.ofNullable(loadingCache.get(orderKey)).orElseThrow(() -> new RuntimeException("Cannot find orders with key"))
            .stream().filter(a -> a.getTaskStatus() == TaskStatus.INIT)
            .filter(Task::isReady)
            .forEach(t -> {
    ...
    private List<Task<I, O>> loadByOrderKey(String orderKey) {
        ...
        Map<Long, Task<I, O>> taskIdMap = taskList.stream().collect(Collectors.toMap(Task::getId, Function.identity()));
        ...
        taskList.forEach(task -> {
            if (Objects.nonNull(task.getSuccessorIds()) && !task.getSuccessorIds().isEmpty()) {
                task.getSuccessorIds().forEach(id -> {
                    ...
                    Task<I, O> successorTask = taskIdMap.get(id);
    ...
    private List<Task<I,O>> loadByTaskId(Long taskId) {
        ...
        return loadingCache.get(orderKey);
    }
    ...
    private final LoadingCache<String, List<Task<I,O>>> loadingCache;
